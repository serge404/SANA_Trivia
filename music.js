$(document).ready(function() {
  ...
  var quiz = {
    ...
    finalFeedbackText: function() {
      ...
      var musicscore = this.musicscore;
      user.providerData.forEach(function (profile) {
                                db.collection("users").doc(profile.displayName).update({musicscore})
        });
      var level = ""
      if (this.musicscore < 5) {
        level = "BEGINNER";
      } else if (this.musicscore < 8) {
        level = "INTERMEDIATE";
      } else {
        level = "ADVANCED";
      }
      return "You got " + this.musicscore + " out of " +
